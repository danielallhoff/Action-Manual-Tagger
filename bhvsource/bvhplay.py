...
from AsyncServer import AsyncServer, ConnectionHandler
...
#Message has three commands: "stop", "play" and "set value"
def read_message(data):
  ...
  if command == "stop":
    ...
    try:
      slidert.set(int(split[1]))
      print("Trying set:")
      print(split[1])
      onSlider(int(split[1]))
    except:
      print "Value setter incorrect!"
  elif command == 'get':
...
def server():
  ...
  global skelscreenedges
  socket_number = 10000
  server_address = ('127.0.0.1', socket_number)
    ...
    try:
        ...
        while True:
            ...
            if data:
                ...
                print str(myskeleton.frames)
                connection.send(str(myskeleton.frames))
            else:
...
#Bind to address(where it listens petitions)
